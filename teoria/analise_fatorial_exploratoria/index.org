...
Após extrair as cargas iniciais, muitas vezes percebemos que as variáveis "carregam" em vários fatores ao mesmo tempo, o que dificulta dizer exatamente o que cada fator significa. É aqui que entra a **Rotação**.

** 6.1 Por que Rotacionar? (O Conceito de Estrutura Simples)

A matriz sem rotação segue um princípio matemático: o primeiro fator deve explicar o *máximo* de variância possível. Isso geralmente cria um "Fator Geral" onde quase tudo tem carga alta.

A rotação busca a **Estrutura Simples (Simple Structure)** de Thurstone:
1.  Cada variável deve ter carga alta em **apenas um** fator.
2.  Cada fator deve ter cargas altas em apenas algumas variáveis e próximas de zero nas outras.
...
O Varimax é o método de rotação **ortogonal** mais comum. Ele "gira" os eixos para que as cargas fiquem próximas de 1, -1 ou 0, eliminando as cargas intermediárias que causam confusão.

No próximo estágio, aplicaremos o cálculo da rotação para ver como a separação entre "Acidez" e "Micronutrientes" fica muito mais nítida.
